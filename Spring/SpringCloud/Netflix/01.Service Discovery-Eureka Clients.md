...
未指定情况，Discovery Client不会向每个Spring Boot Actuator传播当前的健康检查状态。也就意味着成功登记后的Eureka会总是声明该程序application是 'UP' 状态。
开启Eureka健康检测后这种行为可能发起警告，从而上传程序状态给Eureka。这样，其他的每个程序都不会与非'UP'状态的程序通信。
...
eureka:
 client:
  healthcheck:
   enabled: true
```
>`eureka.client.healthcheck.enabled=true`只能被设置在**application.yml**中。 给bootstrap.yml赋值会导致副作用出现,如登记过的eureka出现`UNKNOWN `状态。
